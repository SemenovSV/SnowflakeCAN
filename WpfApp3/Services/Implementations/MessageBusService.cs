...
public class MessageBusService : IMessageBus
{
    private class Subscription<T> : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            var lockup = bus._Lock;
            lockup.EnterWriteLock();
    ...
    private IEnumerable<Action<T>> GetHandlers<T>()
    {
        ...
        try
        {
            ...
            foreach (var r in refs)
                if (r.Target is Subscription<T> { Handler: var handler })
    ...
    public void Send<T>(T msg)
    {
        ...
        foreach (var handler in handlers) handler(msg);
    }
}
